...
{	
	...
	/**
	  * assign a Table for customers who made reservation before <br>
	  * - if the reservation is not found, treat the customers as a walk-in, no reservation is removed <br>
	  * - if the customers come early (there is a chance that the reserved table is occupied), we let the staff decides whether to assign the reserved table or assign new table, and then remove the reservation subsequently <br>
	  * - 	if the staff chooses to assign reserved table and the table is available, the table will be assigned <br>
	  * - 	if the staff chooses to assign reserved table and the table is occupied, the system will assign a new table automatically <br>
	  * - 	if the staff chooses to assign a new table, then a new table will be assigned
	  * - if the customers come on time or no later than the expiry time of the reservation, assign the reserved table as per normal and remove the reservation
	  */
